...
export const Recipes: React.FC = () => {
  ...
  const availableTags: string[] = ['Todos', ...Array.from(new Set(
      recipes
    ...
    <div className="pb-24 md:pb-8 animate-in fade-in duration-500 min-h-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredRecipes.map((recipe) => (
                      <div 
                        ...
                        className="flex gap-4 p-3 bg-white dark:bg-brand-surface rounded-2xl border border-gray-100 dark:border-brand-border cursor-pointer hover:border-brand-accent/30 transition-all active:bg-gray-50 dark:active:bg-white/5"
                      >
                          ...
                          <div className="flex-1 py-1 flex flex-col justify-center">
                              <div className="flex flex-wrap gap-1 mb-1">
                                  {recipe.tags?.slice(0, 2).map(t => (
                                      <span key={t} className="text-[9px] px-1.5 py-0.5 bg-brand-accent/10 text-brand-accent rounded font-bold uppercase">{t}</span>
                                  ))}
                              </div>
                              <h3 className="font-bold text-black dark:text-white leading-tight mb-2 line-clamp-2">{recipe.title}</h3>
                              <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-brand-muted mt-auto">
      ...
      {selectedRecipe && (
          <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-6">
              ...
              <div className="relative bg-brand-dark md:bg-brand-surface w-full md:max-w-2xl h-[90vh] md:h-auto md:max-h-[90vh] md:rounded-3xl rounded-t-3xl overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-300 shadow-2xl">
                  ...
                  {/* Header Image */}
                  <div className="h-64 md:h-64 relative shrink-0">
                      ...
                      <div className="absolute bottom-4 left-6 right-6">
                           <div className="flex gap-2 mb-2">
                               {selectedRecipe.tags?.map(t => (
                                   <span key={t} className="px-2 py-1 rounded bg-brand-accent text-[10px] font-bold text-white uppercase shadow-sm">{t}</span>
                               ))}
                           </div>
                            ...
                            <div className="space-y-4">
                                {selectedRecipe.instructions?.map((step, i) => (
                                    <div key={i} className="flex gap-4">
                                        <span className="text-sm font-bold text-brand-muted/50 pt-1">{i + 1}.</span>
